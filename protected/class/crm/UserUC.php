...
class UserUC extends mainClass
{
    ...
    public function userRegister($merchant_id, $account, $msg_pwd, $pwd, $confirm_pwd)
    {
        ...
        try {
            //参数验证
            //TODO
// 			if (empty($merchant_id) || !isset($merchant_id)) {
// 				$result['status'] = ERROR_PARAMETER_MISS;
// 				throw new Exception('缺少字段merchant_id');
// 			}
...
// 			exit();
            ...
            foreach ($model as $key => $value) {
                ...
                $data['list'][$key]['name'] = $value['coupons']['title'];
                ...
                if ($value['coupons']['type'] == COUPON_TYPE_REDENVELOPE || $value['coupons']['type'] == COUPON_TYPE_CASH) {
                    ...
                    $data['list'][$key]['money'] = $value['coupons']['money'];
                }
...
// 			}
     ...
     * @return string
     */
    public function couponsDetail($id, $user_id)
    {
        $result = array();
        try {
            $user_coupons = UserCoupons::model()->find('id =:id and user_id =:user_id and flag =:flag', array(
                ':id' => $id,
                ':user_id' => $user_id,
                ':flag' => FLAG_NO
            ));
            ...
            if (!empty($user_coupons)) {
                ...
                $data['title'] = $user_coupons->coupons->title;
                $data['vice_title'] = $user_coupons->coupons->vice_title;
                $data['color'] = $user_coupons->coupons->color;
                $data['mini_consumption'] = $user_coupons->coupons->mini_consumption;
                $data['qcode'] = $user_coupons->coupons->code;
                $data['code'] = $user_coupons->code;
                $data['tel'] = $user_coupons->coupons->tel;
                //$data['fixed_value'] = $user_coupons->coupons->exchange;
                $data['discount'] = $user_coupons->coupons->discount;
                $data['discount_illustrate'] = $user_coupons->coupons->discount_illustrate;
                $data['use_illustrate'] = $user_coupons->coupons->use_illustrate;
                //$data['validtime_start'] = $user_coupons->coupons->validtime_start;
                $data['start_time'] = $user_coupons->start_time;
                //$data['validtime_end'] = $user_coupons->coupons->validtime_end;
                $data['end_time'] = $user_coupons->end_time;
                $data['use_num'] = $user_coupons->coupons->receive_num;
                //$data['validtime_fixed_value'] = $user_coupons->coupons->validtime_fixed_value;
                $data['validtime_fixed_value'] = $user_coupons->coupons->effective_days;
            } else {
...
//     	return $result;
    ...
    //判断用户是否已经参与过周福利活动
    public function checkUserIFZfl($merchant_id = '', $encrypt_id = '', $openid,$user_id)
    {
        $result = array();
        try {
            if (!empty($encrypt_id)) {
    ...
    public function checkUserIfLogin($wechat_open_id = '', $alipay_open_id = '', $encrypt_id = '', $merchant_id = '')
    {
        ...
        try {
            ...
            if (!empty($wechat_open_id)) {
                $user = User::model()->find('merchant_id =:merchant_id and flag =:flag and wechat_id =:wechat_id and account is not null', array(
                    ':merchant_id' => $merchant_id,
                    ':flag' => FLAG_NO,
                    ':wechat_id' => $wechat_open_id
                ));
            } elseif (!empty($alipay_open_id)) {
                $user = User::model()->find('merchant_id =:merchant_id and flag =:flag and alipay_fuwu_id =:alipay_fuwu_id', array(
                    ':merchant_id' => $merchant_id,
                    ':flag' => FLAG_NO,
