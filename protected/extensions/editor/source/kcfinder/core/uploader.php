    ...
    public function upload() {
        ...
        if ($config['disabled'] || $config['readonly']) {
            ...
            if (!strlen($message)) {
                ...
                if (!is_dir(path::normalize("$dir{$date_folder}")))
                	@mkdir(path::normalize("$dir{$date_folder}"), $this->config['dirPerms'], true);
                do {
                	//用时间+随机数重命名图片文件
                	$key = date("ymdhis".rand(100000,999999));
                	$file_name = $key.strrchr(basename($file['name']),".");
                	$path = "{$date_folder}/{$file_name}";
                	$target = file::getInexistantFilename("$dir{$path}");
