...
/* 增加成员变量 $wxpay_config
 * 
 * 修改常量数组为变量数组 WxPayConfig -》 $this->wxpay_config
 * 
 * 修改静态方法为实例方法，方法包括：unifiedOrder() orderQuery() closeOrder() refund() refundQuery()
 * downloadBill() micropay() reverse() report() bizpayurl() shorturl() reportCostTime() postXmlCurl() 
 * 
 * 修改调用静态方法为调用实例方法 self:: -》 $this->
 * 方法包括：unifiedOrder() orderQuery() closeOrder() refund() refundQuery()
 * downloadBill() micropay() reverse() report() bizpayurl() shorturl() reportCostTime() postXmlCurl()
 * 
 * 修改调用方法 WxPayResults::Init($response) -》 WxPayResults::Init($response, $this->wxpay_config)
 * 
 * 增加属性设置 WxPayReport()  设置wxpay_config
 */
...
class WxPayApi
{
	...
	public function unifiedOrder($inputObj, $timeOut = 6)
	{
		...
		if(!$inputObj->IsNotify_urlSet()){
			$inputObj->SetNotify_url($this->wxpay_config['NOTIFY_URL']);//异步通知url
		}
		
		$inputObj->SetAppid($this->wxpay_config['APPID']);//公众账号ID
		$inputObj->SetMch_id($this->wxpay_config['MCHID']);//商户号
		...
		$result = WxPayResults::Init($response, $this->wxpay_config);
	...
	public function notify($callback, &$msg)
	{
		...
		//如果返回成功则验证签名
		try {
			$result = WxPayResults::Init($xml, $this->wxpay_config);
	...
	private function reportCostTime($url, $startTimeStamp, $data)
	{
		...
		//增加：设置wxpay_config
		$objInput->wxpay_config = $this->wxpay_config;
	...
	{		
		...
		if($useCert == true){
			...
			$cert_path = $this->wxpay_config['SSLCERT_PATH'];
			curl_setopt($ch,CURLOPT_SSLCERT, $cert_path);
			curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
			$key_path = $this->wxpay_config['SSLKEY_PATH'];
