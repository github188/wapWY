...
/***********************修改备注****************************/
/* 以下类增加了SetSub_appid、GetSub_appid、IsSub_appidSet、SetSub_mch_id、GetSub_mch_id、IsSub_mch_idSet方法
 * 类：WxPayBizPayUrl,WxPayCloseOrder,WxPayDownloadBill,WxPayJsApiPay,WxPayMicroPay,WxPayOrderQuery,
 * WxPayRefund,WxPayRefundQuery,WxPayReport,WxPayReverse,WxPayShortUrl,WxPayUnifiedOrder
 */
...
class WxPayResults extends WxPayDataBase
{
	 ...
	 */
	public static function InitFromArray($array, $wxpay_config, $noCheckSign = false)
	{
		$obj = new self();
		$obj->wxpay_config = $wxpay_config;
		$obj->FromArray($array);
     ...
     * @throws WxPayException
     */
	public static function Init($xml, $wxpay_config)
	{	
		$obj = new self();
		$obj->wxpay_config = $wxpay_config;
		$obj->FromXml($xml);
...
{	
	...
	**/
	...
	/**
	 * 设置trade_type=JSAPI，此参数必传，用户在子商户appid下的唯一标识。下单前需要调用【网页授权获取用户信息】接口获取到用户的Openid。
	 * @param string $value
	 **/
	...
	/**
	 * 获取trade_type=JSAPI，此参数必传，用户在子商户appid下的唯一标识。下单前需要调用【网页授权获取用户信息】接口获取到用户的Openid。 的值
	 * @return 值
	 **/
	...
	/**
	 * 判断trade_type=JSAPI，此参数必传，用户在子商户appid下的唯一标识。下单前需要调用【网页授权获取用户信息】接口获取到用户的Openid。 是否存在
	 * @return true 或 false
	 **/
