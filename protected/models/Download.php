...
class Download extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('admin_id, type, flag', 'numerical', 'integerOnly'=>true),
			array('title, download_url', 'length', 'max'=>100),
			array('publish_to', 'length', 'max'=>10),
			array('create_time, last_time', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, admin_id, title, type, download_url, publish_to, create_time, last_time, flag', 'safe', 'on'=>'search'),
		);
	}
