...
class PointsRule extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('merchant_id, cycle, num, points, if_storedpay_get_points, flag', 'numerical', 'integerOnly'=>true),
			array('condition', 'numerical'),
			array('name', 'length', 'max'=>100),
			array('create_time, last_time', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, merchant_id, name, cycle, num, condition, points, if_storedpay_get_points, create_time, last_time, flag', 'safe', 'on'=>'search'),
		);
	}
