        ...
        <?php foreach ($cart as $k => $v){?>
        	<div class="itemCom clearfix">
                <div class="checkboxCom check"><!--多选框选中加样式名checkboxComCur-->
                    <input type="checkbox" id="checkbox01" class="c_choose" name="skuid[<?php echo $v -> id?>]" value="<?php echo $v -> sku_id;?>"> 
                    <label for="checkbox01">&nbsp;</label>
                </div>
                    <div class="htcon"><img src="<?php $img_arr = explode(';', $v -> img);echo substr($img_arr[0],0,4) == 'http' ? $img_arr[0] : IMG_GJ_80_LIST.$img_arr[0]?>"></div>
                <div class="text">
                    <div class="title">
                    ...
                    <div class="block">
                        ...
                        <div class="num_select">
                            <b>-</b>
                            <input type="text" class="text" maxlength="4" value="<?php echo $v -> num?>" name="num[<?php echo $v -> id?>]"/>
                            <i>+</i>
	...
	$('b').click(function(){
		...
		if(n<1){
            ...
            if(confirm("是否要删除该商品"))
            {
                ...
                var flag=false;;
                $.ajax({
                    url:'<?php echo Yii::app()->createUrl('Dmall/Cart/DelCartGoods')?>',
                    type:'POST',
