...
<header>
    ...
    <div class="top">
        <ul>
            <li class="" id="salesli">
                <a href="<?php echo Yii::app()->createUrl('Dmall/Commodity/commodityList', array('typename' => 'limit_num', 'type' => $type, 'placeholder' => $placeholder, 'shopgorup' => $shopgroup, 'title' => $title, 'groupid' => $groupid, 'encrypt_id' => $encrypt_id)) ?>"
                   class="" id="salesa">销量<em></em></a>
            </li>
            <li class="" id="priceli">
                <a href="<?php echo Yii::app()->createUrl('Dmall/Commodity/commodityList', array('typename' => 'price', 'type' => $type, 'placeholder' => $placeholder, 'shopgorup' => $shopgroup, 'title' => $title, 'groupid' => $groupid, 'encrypt_id' => $encrypt_id)) ?>"
                   class="" id="pricea">价格<em></em></a>
            </li>
            <li class="" id="newgoodsli">
                <a href="<?php echo Yii::app()->createUrl('Dmall/Commodity/commodityList', array('typename' => 'last_time', 'type' => $type, 'placeholder' => $placeholder, 'shopgorup' => $shopgroup, 'title' => $title, 'groupid' => $groupid, 'encrypt_id' => $encrypt_id)) ?>"
            ...
            <div class="list clearfix">
                <?php if (!empty($group)) {
                    ...
                    <a href="<?php echo Yii::app()->createUrl('Dmall/Commodity/commodityList', array('typename' => 'limit_num', 'type' => $type, 'placeholder' => $placeholder, 'shopgorup' => $shopgroup, 'title' => $title, 'groupid' => $v->id, 'encrypt_id' => $encrypt_id)) ?>"
    ...
    <?php if (!empty($data)) {
        foreach ($data as $key => $value) { ?>
            <article class="item clearfix" id="goods">
                <div class="img">
                    <a href="<?php echo Yii::app()->createUrl('Dmall/commodity/commodityDetails', array('id' => $value['id'], 'encrypt_id' => $encrypt_id)) ?>">
                        <?php $img_arr = explode(';', $value['img']) ?>
                        <img src="<?php echo !empty($value['ts_product_id']) ? $img_arr[0] : IMG_GJ_125_LIST . $img_arr[0]; ?>">
                    </a>
                </div>
                <div class="text">
                    <a href="<?php echo Yii::app()->createUrl('Dmall/commodity/commodityDetails', array('id' => $value['id'], 'encrypt_id' => $encrypt_id)) ?>">
                    ...
                    <span class="add"><!-- <a href="#" style="display: none"><img id="imgadd" src="<?php //echo USER_STATIC_IMAGES;?>user/add.png"></a> --></span>
                </div>
...
//        判断是否显示标题
    ...
    $(document).on('click', '#goods', function (e) {
        e.stopPropagation();
        window.location.href = "<?php echo Yii::app()->createUrl('Dmall/commodity/commodityDetails')?>?id=" + $(this).find('#id').val() + "&encrypt_id" + encrypt_id;
    });
    ...
    $(document).on('keydown', '#enter', function (e) {
        var encrypt_id = '<?php echo $encrypt_id?>';
        var title = $('#goods_title').val();
        var url = "<?php echo Yii::app()->createUrl('Dmall/commodity/commodityList')?>?typename=" + $(document).find('#typename').val() + "&type=" + $(document).find('#type').val() + "&placeholder=" + $(document).find('#enter').val() + "&shopgroup=" + $(document).find('#shopgroup').val() + "&title=" + title + "&encrypt_id=" + encrypt_id;
        if (e.keyCode == 13) {
    ...
    $(document).on('click', '#imgadd', function (e) {
        ...
        $.ajax({
            url: '<?php echo(Yii::app()->createUrl('Dmall/Commodity/GetProductStandard'));?>',
            data: {id: id},
            type: 'post',
            dataType: 'json',
            success: function (data) {
                if (data.status == <?php echo ERROR_NONE?>) {
                    ...
                    $('.genre').bind("click", function () {
                        ...
                        $.ajax({
                            url: '<?php echo(Yii::app()->createUrl('Dmall/Commodity/GetProductSkuPrice'));?>',
...
//            输入框内的数字=1，设置减号为不可点击
    ...
    $('#addcartnow').click(function () {
        ...
        $.ajax({
            ...
            data: {sku_name: sku_name, num: num, product_id: product_id},
            type: 'post',
            dataType: 'json',
            success: function (data) {
                if (data.status == <?php echo ERROR_NONE?>) {
                    ...
                    if (data.status == <?php echo ERROR_PARAMETER_FORMAT?>) {
                        window.location.href = "<?php echo Yii::app()->createUrl('mobile/auth/login', array('goUrl' => Yii::app()->createUrl('Dmall/Commodity/CommodityList'), 'encrypt_id' => $encrypt_id));?>";
                    } else {
                        alert(data.errMsg);
                    }
                }
...
//                 alert(JSON.stringify(res));
        ...
        wx.onMenuShareTimeline({
            title: '【东钱湖旅游】商城列表', // 分享标题
            link: '<?php echo WAP_DOMAIN . '/Dmall/Commodity/CommodityList?encrypt_id=' . $encrypt_id;?>', // 分享链接
