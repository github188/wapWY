        ...
        <?php } elseif ($v->name == 'img_adv') {//图片广告?>
            ...
            <div class="picAD">
                ...
                <a href="<?php if ($arr[2] == 1) {
                    echo Yii::app()->createUrl('mall/Commodity/CommodityList', array('groupid' => $arr[0], 'encrypt_id' => $encrypt_id));
                } elseif ($arr[2] == 2) {
                    echo Yii::app()->createUrl('mall/Commodity/commodityDetails', array('id' => $arr[0], 'encrypt_id' => $encrypt_id));
                } ?>">
                <?php } else { ?>
                    <a><img src="<?php echo IMG_GJ_600_LIST . $v->url[0]; ?>"></a>
                <?php } ?>
            </div>
                            ...
                            <?php if ($y != 'null') { ?>
                                <li>
                                    <a href="<?php if ($v->href[$x] != 'null') {
                                        ...
                                        if ($arr[2] == 1) {
                                            echo empty($arr[0]) ? '' : Yii::app()->createUrl('mall/Commodity/CommodityList', array('groupid' => $arr[0], 'encrypt_id' => $encrypt_id));
                                        } elseif ($arr[2] == 2) {
                                            echo empty($arr[0]) ? '' : Yii::app()->createUrl('mall/Commodity/commodityDetails', array('id' => $arr[0], 'encrypt_id' => $encrypt_id));
                                        }
                                    } ?>">
        ...
        <?php } elseif ($v->name == 'shop_group') {//商品分组?>
            <?php foreach ($v->href as $x => $y) { ?>
                ...
                <?php if ($y != 'null' && !empty($groups[$arr[0]]['product'])) { ?>
                    <article class="title">
                        <a href="<?php echo Yii::app()->createUrl('mall/Commodity/CommodityList', array('groupid' => $arr[0], 'encrypt_id' => $encrypt_id)); ?>">
                    ...
                    foreach ($groups[$arr[0]]['product'] as $n => $m) { ?>
                        ...
                        <article class="item clearfix">
                            <div class="img">
                                <a href="<?php echo Yii::app()->createUrl('mall/commodity/commodityDetails', array('id' => $m['id'], 'encrypt_id' => $encrypt_id)) ?>">
        ...
        <div class="pop_con">
            <dl>
                <?php foreach ($groups as $key => $v) { ?>
                    <dt>
                        <a href="<?php echo Yii::app()->createUrl('mall/Commodity/commodityList', array('typename' => 'limit_num', 'groupid' => $key, 'encrypt_id' => $encrypt_id)) ?>"><?php echo $v['group_name']; ?></a>
                        <span class="jt"></span>
                    </dt>
                <?php } ?>
            </dl>
...
<script type="text/javascript">
    ...
    $(document).on('keydown', '#enter', function (e) {
        ...
        var url = "<?php echo Yii::app()->createUrl('mall/commodity/commodityList')?>?typename=limit_num&type=up&placeholder=" + value + "&shopgroup=&title=&encrypt_id=" + encrypt_id;
        if (e.keyCode == 13) {
...
//                 alert(JSON.stringify(res));
        ...
        wx.onMenuShareTimeline({
            title: '商城首页', // 分享标题
            link: '<?php echo WAP_DOMAIN . '/mall/Commodity/index?encrypt_id=' . $encrypt_id;?>', // 分享链接
