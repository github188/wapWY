...
<section class="set-con">
    ...
    <div class="bottom">
    	<?php if ($book['status'] == BOOK_RECORD_STATUS_WAIT || $book['status'] == BOOK_RECORD_STATUS_ACCEPT) {?>
	    	<span><a href="<?php echo Yii::app()->createUrl('mobile/book/book/bookCancel', array('record_id'=>$book['id'], 'encrypt_id' => $encrypt_id)) ?>" class="btn-com-gray" onclick="if(confirm('确定取消?')==false)return false;" >取消订单</a></span>
	    <?php }?>
    </div>
</section>
</body>
