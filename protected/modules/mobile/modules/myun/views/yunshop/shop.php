...
<?php if (!empty($shop)) { ?>
    ...
    <header class="top_title">
        <div class="banner">
            <a href="<?php echo Yii::app()->createUrl('mobile/myun/yunshop/Album', array('encrypt_id' => $encrypt_id)) ?>">
    ...
    <article>
        <section class="sh-con">
            ...
            <div class="item item_10">
                <a class="intergral" href="tel:<?php echo $store[$store_id]['telephone']?>">
                    <span class="ico"></span><?php echo $store[$store_id]['telephone'] ?><span class="jt"></span>
                </a>
            </div>
            <?php if (count($store) > 1) { ?>
                <div class="more">
                    <a href="<?php echo Yii::app()->createUrl('mobile/uCenter/user/storeList', array('model' => json_encode($store), 'encrypt_id' => $encrypt_id)) ?>">更多门店</a>
                </div>
            <?php } ?>
        </section>
        ...
        <section class="item item_14">
            <a class="intergral"
               href="<?php echo Yii::app()->createUrl('mobile/uCenter/user/memberCenter', array('encrypt_id' => $encrypt_id)) ?>">
                <span class="ico"></span>会员中心<span class="jt"></span>
            </a>
        </section>

        <?php if ($shop['if_book'] == MERCHANT_BOOK_OPEN) { ?>
            <section class="item item_8" style="<?php if (!empty($online)) {
                ...
                <a class="intergral"
                   href="<?php echo Yii::app()->createUrl('mobile/book/book/bookOperate', array('store_id' => $online['store_id'], 'encrypt_id' => $encrypt_id)) ?>">
                    <span class="ico"></span>预订<span class="jt"></span>
                </a>
            </section>
        <?php } ?>

        <?php if ($shop['if_stored'] == MEMBERSHIP_STORED_STATUS_OPEN) { ?>
            ...
            <section class="list">
                ...
                <?php if ($count_stored > 2) { ?>
                    <div class="more">
                        <a href="<?php echo Yii::app()->createUrl('mobile/uCenter/user/list', array('title' => 'stored', 'model' => $stored, 'encrypt_id' => $encrypt_id)) ?>">更多储值活动</a>
                    </div>
                <?php } ?>
            </section>
        <?php } ?>

        <?php if ($shop['if_coupons'] == MERCHANT_COUPONS_OPEN) { ?>
            ...
            <section class="list">
                ...
                <?php if ($count_coupons > 2) { ?>
                    <div class="more">
                        <a href="<?php echo Yii::app()->createUrl('mobile/uCenter/user/list', array('title' => 'coupons', 'model' => json_encode($coupons['coupons']), 'encrypt_id' => $encrypt_id)) ?>">更多优惠券</a>
                    </div>
                <?php } ?>
            </section>
        <?php } ?>
...
<script type="text/javascript">
    function changeStore() {
        ...
        window.location.href = "<?php echo Yii::app()->createUrl('mobile/myun/yunshop/shop')?>" + "?store_id=" + store_id + "&encrypt_id=" + '<?php echo $encrypt_id; ?>' +"&source=" + source;
    }
</script>
