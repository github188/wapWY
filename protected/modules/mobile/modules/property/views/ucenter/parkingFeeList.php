<div class="prop_wrap prop_property">
    <div class="weui_tab weui_tab35">
        <div class="weui_navbar">
            ...
            <a class="weui_navbar_item weui_bar_item_on" onclick="window.location.href= '<?php echo Yii::app()->createUrl('mobile/property/Ucenter/parkingFeeList',array('encrypt_id' => $this->getEncryptId()));?>'">
    ...
    <?php if(!empty($list)){?>
        <?php foreach($list as $k => $v){?>
            <?php if(!empty($v['parking_month_num'])){?>
                <div class="weui_panel weui_panel_access defined_panel" onclick="window.location.href= '<?php echo Yii::app()->createUrl('mobile/property/fee/parkingFeeView', array('id' => $v['id'], 'encrypt_id' => $encrypt_id));?>' ">
...
<!--                                预存金额：<em>--><?php //echo isset($v['money']) ? '￥' .  $v['money'] : '0.00';?><!--</em>-->
                            <span class="cGray">
                                <?php if($v['parking_month_num'] == 6) {
                                    echo '半年';
                                } elseif($v['parking_month_num'] == 12) {
                                    echo '一年';
                                }else{
                                    echo $v['parking_month_num'];echo '个月';
                                } ?>
                            </span>
                            </p>
                        </div>
