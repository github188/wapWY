...
<section class="content">
    <div class="hd">
        ...
        <div class="coupon">
            <h1><?php echo $GLOBALS['COUPON_TYPE'][$coupons['type']] ?></h1>
            <!-- 红包 代金券 -->
            <?php if ($coupons['type'] == COUPON_TYPE_REDENVELOPE || $coupons['type'] == COUPON_TYPE_CASH) { ?>
                <?php if (!empty($coupons['fixed_value'])) { ?>
                    <span><?php echo $coupons['fixed_value'] ?>元</span>
                <?php } else { ?>
                    <span><?php echo $coupons['userdefined_value'] ?>元随机金额</span>
                <?php } ?>
        ...
        <div class="op">
            <a href="<?php echo Yii::app()->createUrl('mobile/uCenter/user/receiveCoupons', array('coupons_id' => $coupons['id'], 'coupons_type' => $coupons['type'], 'encrypt_id' => $encrypt_id)) ?>"
               class="btn">免费领取</a>
        </div>
    </div>
