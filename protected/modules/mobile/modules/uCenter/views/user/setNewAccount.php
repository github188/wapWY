...
<script language="JavaScript">
    ...
    $('input[name = User\\[MobilePhone\\]]').bind('blur', function () {
        $.post('<?php echo(Yii::app()->createUrl('mobile/uCenter/user/isexist'));?>?', "account=" + $(this).val() + "&encrypt_id=<?php echo $encrypt_id ?>", function (data) {
...
<script language="JavaScript">
    ...
    function onMobileMsg() {
        ...
        if (!reg.test(mobile)) {
            ...
            $.ajax({
                url: '<?php echo Yii::app()->createUrl('mobile/uCenter/user/sendMsgPassword');?>?' + new Date().getTime(),
