...
<script>
    	...
    	$.ajax({
            ...
            success: function (data) {
                if(data.error == 'success') {
                	...
                	location.href = "<?php echo Yii::app()->createUrl('syt/cashier/success',array('orderNo'=>$_GET['orderNo']))?>";
                }else if(data.error == 'jump') {
                	location.href = "<?php echo Yii::app()->createUrl('syt/cashier/wait')?>"+"?orderNo="+orderNo;
    	...
    	$.ajax({
            ...
            success: function (data) {
                if(data.error == 'success') {
                    ...
                    var time = data.time;
                	printQRcode(pic,image,mName,sName,operator,orderNo,type,status,money,time,printname);
                }else {
                	alert(data.errMsg);
                }
            }
    ...
    function printQRcode(pic,image,mName,sName,operator,orderNo,type,status,money,time,printname) {
        ...
        LODOP.SET_PRINT_STYLEA(0,"FontName",'宋体');//打印字体型号
        ...
        LODOP.ADD_PRINT_IMAGE(229,curX+50,170,30,"<img src="+image+" width='' height='30'>");
        ...
        var x = 0;
        if(mName.length > 9){
        	LODOP.ADD_PRINT_TEXT(curY,curX+5,200,30,"商户名称 "+mName.substr(0,9));
        	x += 16;
        	curY += x;
        	LODOP.ADD_PRINT_TEXT(curY,curX+5+50,200,30,mName.substr(9));
        }else{
        ...
        var y = 0;
        if(sName.length > 9){
        	LODOP.ADD_PRINT_TEXT(curY,curX+5,200,30,"门店名称 "+sName.substr(0,9)); 
        	y += 16;
        	curY += y;
        	LODOP.ADD_PRINT_TEXT(curY,curX+5+50,200,30,sName.substr(9)); 
        }else{
		...
		curY += textSpace / 2;
		if(money.length > 8) {
			LODOP.ADD_PRINT_TEXT(curY,curX+60,125,45,"￥ "+money);
			LODOP.SET_PRINT_STYLEA(0,"Alignment",1);
		}else {
			LODOP.ADD_PRINT_TEXT(curY,curX+60,125,30,"￥ "+money);
