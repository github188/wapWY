...
<body class="order">
	<header class="top_title">
		<nav class="header-nav">
			<ul>
				<li class="<?php echo ORDER_PAY_WAITFORCONFIRM == $stored_confirm_status ? 'bg' : '';?>"><a href="<?php echo Yii::app()->createUrl('uCenter/user/orderList', array('stored_confirm_status'=>ORDER_PAY_WAITFORCONFIRM, 'encrypt_id' => $encrypt_id)) ?>">待确认</a></li>
				<li class="<?php echo ORDER_PAY_WAITFORCONFIRM != $stored_confirm_status ? 'bg' : '';?>"><a href="<?php echo Yii::app()->createUrl('uCenter/user/orderList', array('stored_confirm_status'=>ORDER_PAY_CONFIRM, 'encrypt_id' => $encrypt_id)) ?>">已完成</a></li>
			</ul>
		</nav>
	</header>
	<section class="mid_con">
		<?php if (!empty($list)) { ?>
			<?php foreach ($list as $key => $value) {?>
				...
				<div class="orderid">
					<div class="id" onclick="detail('<?php echo $value['id'] ?>')">
...
<script type="text/javascript">
	function detail(order_id){
		location.href="<?php echo Yii::app()->createUrl('uCenter/user/orderDetail'); ?>" + "?order_id=" + order_id;
	}
	$(".btn-pay").click(function() {
		...
		$.ajax({
            url: '<?php echo(Yii::app()->createUrl('uCenter/user/storedConfirm'));?>',
