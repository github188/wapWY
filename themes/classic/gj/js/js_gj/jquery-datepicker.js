...
(function ($) {
    ...
    var months = {
        ...
        todayDate = new Date(),
        todayYear = todayDate.getFullYear() - 20,
    ...
    $.fn.birthdaypicker = function (options) {
        ...
        return this.each(function () {
            ...
            var $wraper = $(_wrap),
                $year = $("<select class='birth-year year' name='birth[year]'></select>"),
                $month = $("<select class='birth-month month' name='birth[month]'></select>"),
                $day = $("<select class='birth-day date' name='birth[day]'></select>");

            if (settings["legend"]) {
                $("<span>" + settings["legend"] + "</span>").appendTo($wraper);
            }
            var $_year = $("<span>" + "年" + "</span>");
            var $_month = $("<span>" + "月" + "</span>");
            var $_day = $("<span>" + "日" + "</span>");

            var tabindex = settings["tabindex"];

            // Deal with the various Date Formats
            if (settings["dateFormat"] == "bigEndian") {
                $wraper.append($year).append($_year).append($month).append($_month).append($day).append($_day);
                if (tabindex != null) {
...
//                $("<option value='0'>日</option>").appendTo($day);
            ...
            for (var i = startYear; i >= endYear; i--) {
                if (i == startYear) {
                    $("<option></option>").attr("value", i).attr("selected", "selected").text(i).appendTo($year);
                } else {
