...
$(function() {
	$(".liTitle").click(function() {
		...
		$(this).find(".downArrow").toggleClass("upArrow");
	})
	...
	/*$(".swipeCon").dropload({
        ...
        domDown: {
			...
			domRefresh : '<div class="dropload-refresh">↑继续拖动，查看商品图文详情</div>',
        ...
        loadDownFn:function(me){
            $.ajax({
                type: 'Get',
                url: 'http://ximan.github.io/dropload/examples/json/more.json',
                dataType: 'json',
                success: function(data){
                   var title = data.lists[1].title;
                   $(".swipeCon p").html(title);
                   me.resetload();
                },
...
$(function(){
	if ($(document).width()>=780) {
		$(".q_header img").attr("src","/themes/classic/gj/user/images/q2_big.jpg");
		$(".q_wrap").css("background-image","url(/themes/classic/gj/user/images/q3_big.jpg)");
		$(".q_contact .q_contact_item input").css("width","300px");
	}

	$("#submit").click(function(){
		for (var i = 0; i < 3; i++) {
			var arr=[];
			console.log($(".q_contact input[0]").val());
		}
