...
var FormValid = function(frm) {
	...
	this.limit = function (inputObj) {
		...
		var len = inputObj.value.replace(/[^\x00-\xff]/g, "**").length;
		if (len) {
...
FormValid.showError = function(errMsg,errName) {
	...
	for (i = 0; i < errMsg.length; i++) {
		...
		document.getElementById('errMsg_' + errName[i]).style.display = '';
	}
}
...
function fvCheck(e,fv,formElements) {
	...
	for (var j=0; j<vts.length; j++) {
		...
		case 'custom':
			...
			break;
		case 'custom1':
			validResult = eval(e.getAttribute('custom1')+'(e,formElements)');
			break;
		default :
